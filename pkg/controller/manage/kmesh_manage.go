...
func NewKmeshManageController(client kubernetes.Interface) error {
	...
	if _, err := podInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			skip, err := checkNamespaceLabels(client, pod, LabelSelectorKmesh)
			if err != nil {
				log.Printf("error checking namespace labels: %v", err)
				return
			}
			if skip {
				return
...
func checkNamespaceLabels(client kubernetes.Interface, pod *corev1.Pod, labelSelector string) (bool, error) {
	namespace, err := client.CoreV1().Namespaces().Get(context.TODO(), pod.GetNamespace(), metav1.GetOptions{})
	if err != nil {
		return false, fmt.Errorf("failed to get namespace %s, err: %v", pod.GetNamespace(), err)
	}

	if _, ok := namespace.Labels[labelSelector]; ok {
		log.Printf("namespace %s has label %s, skipping further processing", pod.GetNamespace(), labelSelector)
		return true, nil
	}
