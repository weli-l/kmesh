...
func StartByPassController(client kubernetes.Interface) error {
	...
	if _, err := podInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			if enableKmesh {
				log.Infof("enableKmesh!,, bypass kmesh,%s", nspath)
				bypassKmeshControl(nspath)
			}
		},
		DeleteFunc: func(obj interface{}) {
			...
			if enableKmesh {
				log.Infof("enableKmesh!,, enable kmesh,%s", nspath)
				enableKmeshControl(nspath)
			}
		},
	}); err != nil {
...
func enableKmeshControl(ns string) error {
	execFunc := func(netns.NetNS) error {
		 ...
		 * of the current connection. The cookie can be used
		 * to determine whether the netns is managed by Kmesh.
		 * 0.0.0.1:931(0x3a4) is "cipher key" for cgroup/connect4
		 * ebpf program.
		 */
		simip := net.ParseIP("0.0.0.1")
		sockfd, err := syscall.Socket(syscall.AF_INET, syscall.SOCK_STREAM, 0)
