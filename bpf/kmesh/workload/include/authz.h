...
int matchSrcIPs(struct xdp_md *ctx)
{
    ...
    match = kmesh_get_ptr_val(res->match);
    if (!match) {
        return XDP_PASS;
    }
...
#pragma unroll   
		for (i = 0; i < MAX_MEMBER_NUM_PER_POLICY; i++) {
			...
			if (matchIp(&srcAddr->address, srcAddr->length, &tuple_key, TYPE_SRCIP) == MATCHED) {
				if (res->action == AUTH_DENY) {
                    return XDP_PASS;
                } else {
                    BPF_LOG(INFO, AUTH, "Denied: srcip matches \n");
                    return XDP_DROP;
                }
			}  
		}
	...
	if (match->n_source_ips != 0) {
		...
		if (!srcPtrs) {
			BPF_LOG(ERR, AUTH, "failed to get srcip ptr\n");
			return (res->action == AUTH_DENY) ? XDP_PASS : XDP_DROP;
		}
...
SEC("xdp_auth")
int matchDstIPs(struct xdp_md *ctx)
{
    return 0;
}
#endif
...
#pragma unroll
    ...
    bpf_tail_call(ctx, &xdp_tailcall_map, TAIL_CALL_SRCIP_MATCH);
    return XDP_PASS;
}
